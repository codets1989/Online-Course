...
function FacultyLogin()
{
    ...
    return(
        <div className="container">
            <div className="title">{t("Faculty Login")}</div>
      <div className="form">
      <form className="form1" action="http://localhost:8000/faculty/login" method="POST" onSubmit={loghandler} >
          
          <div>
            <label htmlFor="Username">{t("Username")}</label>
            <input type ="text" name="username" id="username"  className='regin' value={userregis.username} onChange={handleinput}  placeholder={t("Enter your Username")} />
        </div>
        <div>
            <label htmlFor="password">{t("Password")}</label>
            <input type ="password" name="password" id="password" className='regin'  value={userregis.password}  onChange={handleinput}  placeholder={t("Enter your password")}/>
        </div>
        
       <button className="form2" type="submit" >{t("Login")}</button>
       <br></br>
      </form>
      <li>  <a href="/Login">{t("For User login click here")}</a> </li>
        </div>
